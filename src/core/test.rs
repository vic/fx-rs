...
fn modify_and_continue_delimited_continuation() {
    let e = Ability::request(5).map(|n| n * 2);
...
#[test]
fn ctx_map_ability() {
    let e = Ability::request(1u8).then(Ability::request(2u8));
...
fn rw_ability() {
    ...
    let e = Ability::request(read).then(Ability::request(write));
...
fn fold_outcome() {
    ...
    let h = Ability::new(|n: usize| Fx::value(n * 2)).fold(0, |acc, i| Fx::value(acc + i));
    let e = h.handle(e);
    let v = e.eval();
...
fn acc_outcome() {
    ...
    let h = Ability::new(Fx::value).fold(2u8, |acc, f: F| f.apply(acc));
